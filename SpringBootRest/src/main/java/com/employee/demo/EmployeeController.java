...
public class EmployeeController {
	...
	public void getAllEmp() throws InterruptedException {
		...
		//return "Cowin App is up and running";
	}
	public  void checkVaccination(String pin) throws InterruptedException {
		try {
			Map centers = (Map<String, Object>) given().baseUri("https://cdn-api.co-vin.in/api/v2/appointment/sessions").when().get("/public/calendarByPin?pincode=" + pin + "&date=" + DateTimeFormatter.ofPattern("dd-MM-yyyy").format(LocalDate.now())).then().log().all().extract().response().body().jsonPath().getJsonObject("$");
			count=0;
			List<Map<String, Object>> arr = (List<Map<String, Object>>) centers.get("centers");
			for (Object ele : arr) {
				List<Map<String, Object>> sessions = (List<Map<String, Object>>) ((Map<String, Object>) ele).get("sessions");
